...
.Aktivierung von Anti-Ransomware
Sie können die Funktionsweise der Aktivierung des Autonomous Ransomware Protection (ARP) steuern. Dies umfasst das Abrufen und Ändern der Konfigurationseinstellungen. Dieser Ressourcentyp wurde mit ONTAP 9.18.1 eingeführt.
...
.Cluster-Netzwerksicherheit
Sie können die Sicherheitskonfiguration des Clusternetzwerks einschließlich der Zertifikate abrufen und aktualisieren. Dieser Ressourcentyp wurde mit ONTAP 9.18 eingeführt.
...
.HA-Netzwerksicherheit
Sie können die HA-Netzwerksicherheitskonfiguration abrufen und aktualisieren. Dieser Ressourcentyp wurde mit ONTAP 9.18 eingeführt.
...
.IPsec-Richtlinien
Mit diesem Satz von API-Aufrufen lassen sich die für eine IPSec-Bereitstellung geltenden Richtlinien verwalten. Dieser Ressourcentyp ist neu mit ONTAP 9.8. Mit 9.19 wurden sie aktualisiert, um die für das Clusternetzwerk verwendeten Richtlinien abzurufen.

.IPsec-Sicherheitszuordnungen
Mithilfe dieses Satzes von API-Aufrufen lassen sich die für eine IPSec-Bereitstellung geltenden Sicherheitszuordnungen verwalten. Dieser Ressourcentyp ist neu mit ONTAP 9.8. Mit 9.19 wurden sie aktualisiert, um die für das Clusternetzwerk verwendeten Sicherheitszuordnungen abzurufen.
...
Web-Authentifizierung (WebAuthn) ist ein Webstandard zur sicheren Authentifizierung von Benutzern auf Basis von Public-Key-Kryptografie. Mit ONTAP wird die Verwaltung von phishing-resistenten MFAs über System Manager und die ONTAP REST API unterstützt. Diese Funktion wurde mit ONTAP 9.16 hinzugefügt und mit ONTAP 9.19 aktualisiert.

.Whoami
Es steht ein Endpunkt zur Verfügung, der die Details des authentifizierten Benutzers anzeigt, einschließlich Benutzername, Rolle und Berechtigungen. Dieser Ressourcentyp wurde mit ONTAP 9.19 eingeführt.
