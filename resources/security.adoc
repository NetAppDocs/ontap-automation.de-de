...
Web-Authentifizierung (WebAuthn) ist ein Webstandard zur sicheren Authentifizierung von Benutzern auf Basis von Public-Key-Kryptografie. Mit ONTAP unterstützt sie die Verwaltung von phishing-resistenten MFAs über System Manager und die ONTAP REST API. Diese Funktion wurde mit ONTAP 9.16 eingeführt und mit ONTAP 9.19.1 aktualisiert.

.Whoami
Es steht ein Endpunkt zur Verfügung, der die Details des authentifizierten Benutzers anzeigt, einschließlich Benutzername, Rolle und Berechtigungen. Dieser Ressourcentyp wurde mit ONTAP 9.19.1 eingeführt.
