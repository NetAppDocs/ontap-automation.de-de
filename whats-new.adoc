...
Mit der ONTAP Version 9.18.1 werden die Funktionen der ONTAP REST API um elf neue API-Aufrufe erweitert. Die Verbesserungen konzentrieren sich in erster Linie auf die Sicherheit, beinhalten aber auch eine Optimierung im Bereich der Speicherverwaltung.

Sicherheit:: Mit der ONTAP 9.18.1 REST-API wurden zwei wichtige Sicherheitsverbesserungen eingeführt. Sie können die neuen Cluster-Netzwerk-API-Endpunkte verwenden, um die Cluster-Netzwerksicherheitskonfiguration anzuzeigen und zu aktualisieren. Eine robuste Unterstützung für die Zertifikatsverwaltung im Clusternetzwerk ist integriert. Darüber hinaus können Sie die Sicherheitskonfiguration des HA-Netzwerks für NVLog-Datenverkehr anzeigen und aktualisieren.
