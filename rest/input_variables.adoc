...
Bei einem API-Aufruf mit GET zum Zugriff auf eine Sammlung von Objekten desselben Typs versucht ONTAP, basierend auf zwei Einschränkungen (maximale Anzahl von Objekten, Zeitaufwand für den Abruf der Objekte), so viele Objekte wie möglich zurückzugeben. Jede dieser Einschränkungen lässt sich mithilfe zusätzlicher Abfrageparameter in der Anfrage steuern. Die zuerst erreichte Einschränkung bei einer bestimmten GET-Anfrage beendet die Anfrage und begrenzt somit die Anzahl der zurückgegebenen Datensätze. Wenn eine Anfrage endet, bevor alle Objekte durchlaufen wurden, enthält die Antwort den Link, der zum Abrufen des nächsten Satzes von Datensätzen erforderlich ist.


TIP: Wenn für einen API-Aufruf null Datensätze zurückgegeben werden und kein Next-Link vorhanden ist, kann dies darauf hinweisen, dass eine Zeitüberschreitung aufgetreten ist, bevor Datensätze abgerufen werden konnten. In diesem Fall kann die Anfrage mit einem größeren `return_timeout` Wert oder einem kleineren `max_records` Wert (oder beidem) erneut gestellt werden.
