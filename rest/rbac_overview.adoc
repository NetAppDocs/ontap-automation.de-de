...
NOTE: Die RBAC-Funktion von ONTAP wurde kontinuierlich erweitert und in ONTAP 9.11.1 (und nachfolgenden Versionen) deutlich verbessert. Siehe link:../rest/rbac_overview.html#summary-of-rbac-evolution["Zusammenfassung der Entwicklung der RBAC"] Und link:../whats-new.html["Was ist neu mit der ONTAP REST API und Automatisierung"] Finden Sie weitere Informationen.
